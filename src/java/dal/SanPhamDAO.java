...
public class SanPhamDAO extends DBContext {
    ...
    public ArrayList<SanPham> getAllProductByCid(int cid) {
        try {
            String sql = "select * from [San_Pham] where cid = ?";
            PreparedStatement st = connection.prepareStatement(sql);
            st.setInt(1, cid);
            ResultSet rs = st.executeQuery();
            ArrayList<SanPham> list = new ArrayList<>();
    ...
    //27/11/2023
    public ArrayList<SanPham> filter(int cid, Map<String, Object> map) {
        try {
            // Tạo một StringBuilder để xây dựng câu lệnh SQL
            StringBuilder sqlBuilder = new StringBuilder();
            sqlBuilder.append("SELECT sp.* FROM San_Pham sp JOIN Thong_So ts ON sp.id = ts.pid WHERE cid = ? ");

            // Tạo một danh sách để lưu trữ các tham số để set vào PreparedStatement
            List<Object> params = new ArrayList<>();
            params.add(cid);

            // Duyệt qua map để xây dựng điều kiện SQL
            for (Map.Entry<String, Object> entry : map.entrySet()) {
                String key = entry.getKey();
                Object value = entry.getValue();

                // Kiểm tra nếu value là một đối tượng Pair (giả sử sử dụng thư viện javafx.util.Pair)
                if (value instanceof Pair pair) {
                    sqlBuilder.append("AND sp.gia_ban BETWEEN ? AND ? ");
                    params.add(pair.getFirst());
                    params.add(pair.getSecond());
                } else {
                    // Các điều kiện khác
                    sqlBuilder.append("AND LOWER(ts.").append(key).append(")").append(" LIKE ? ");
                    if (key.equals("cpu")) {
                        params.add("%" + value + "%");
                    } else {
                        params.add(value + "%");
